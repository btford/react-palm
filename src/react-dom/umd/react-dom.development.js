...
function invokeGuardedCallback(name, func, context, a, b, c, d, e, f) {
  hasError = false;
  caughtError = null;
  return invokeGuardedCallbackImpl$1.apply(reporter, arguments);
}
...
function invokeGuardedCallbackAndCatchFirstError(name, func, context, a, b, c, d, e, f) {
  var returned = invokeGuardedCallback.apply(this, arguments);
  if (hasError) {
...
function executeDispatch(event, listener, inst) {
  ...
  var returned = invokeGuardedCallbackAndCatchFirstError(type, listener, undefined, event);
...
var reportStuffReturnedFromEventHandlers = function (values) {};
...
var executeDispatchesAndRelease = function (event) {
  if (event) {
    var returned = executeDispatchesInOrder(event);
...
var injection = {
  ...
  injectListenToHandlers: function (fn) {
...
function runEventsInBatch(events) {
  ...
  forEachAccumulated(processingEventQueue, function (event) {
    handlerReturnValues = handlerReturnValues.concat(executeDispatchesAndReleaseTopLevel(event));
  });
...
var ReactDOM = {
  ...
  inject: function (onHandler) {
    injection.injectListenToHandlers(onHandler);
  },
