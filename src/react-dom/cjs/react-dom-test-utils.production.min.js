...
var W = {
  ...
  scryRenderedDOMComponentsWithClass: function(a, b) {
    ...
    return W.findAllInRenderedTree(a, function(a) {
      if (W.isDOMComponent(a)) {
        ...
        var d = c.split(/\s+/);
        Array.isArray(b) ||
          (void 0 === b ? p('11') : void 0, (b = b.split(/\s+/)));
  ...
  findRenderedDOMComponentWithTag: function(a, b) {
    ...
    if (1 !== a.length)
      throw Error(
        'Did not find exactly one match (found: ' + a.length + ') for tag:' + b
      );
    return a[0];
  },
...
function ba(a) {
  return function(b, c) {
    ...
    e.phasedRegistrationNames
      ? Q.accumulateTwoPhaseDispatches(f)
      : Q.accumulateDirectDispatches(f);
...
function X() {
  W.Simulate = {};
  var a = void 0;
  for (a in N.eventNameDispatchConfigs) W.Simulate[a] = ba(a);
}
...
function ea(a, b) {
  return function(c, e) {
    ...
    g(d, e);
    W.isDOMComponent(c)
      ? ((c = K(c)), (d.target = c), T.dispatchEvent(b, d))
      : c.tagName && ((d.target = c), T.dispatchEvent(b, d));
  };
}
[
  ['abort', 'abort'],
